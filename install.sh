...
sudo apt-get install -y openssh-server ufw nano git python3-venv python3-pip
...
pip install flask paramiko flask_socketio flask_login
...
echo -e "Остановка: sudo systemctl stop site.service   sudo kill -9 $(sudo lsof -t -i:80)"
